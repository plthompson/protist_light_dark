...
load("./data/pop_data2.RData")

pop_data <- pop_data2 %>% filter(time >0, time < 5) %>% droplevels() %>% ungroup() %>%  select(-file, -date)
pop_data_0 <- pop_data2 %>% filter(time == 0) %>% droplevels() %>% ungroup() %>%  select(-file, -date)
...
monocultures <- pop_data %>% 
  filter(code != "mix1", code != "mix2") %>% 
  group_by(temporal_change,dispersal, replicate, time, predicted_species, environment_1) %>% 
  summarise(N = sum(N)) %>% 
  mutate(code = "monoculture")

monocultures <- monocultures %>% 
  ungroup() %>% 
  droplevels() %>% 
  complete(temporal_change, dispersal, replicate, time, predicted_species, environment_1, code,fill = list(N = 0))

polycultures1 <- pop_data %>% 
  filter(code == "mix1") %>% 
  group_by(temporal_change,dispersal, replicate, time, predicted_species, code, environment_1) %>% 
  summarise(N = sum(N)) %>%
  ungroup() %>% 
  mutate(code = "polyculture")

polycultures1 <- polycultures1 %>% 
  ungroup() %>% 
  droplevels() %>% 
  complete(temporal_change, dispersal, replicate, time, predicted_species, environment_1, code ,fill = list(N = 0))


polycultures2 <- pop_data %>% 
  filter(code == "mix2") %>% 
  group_by(temporal_change,dispersal, replicate, time, predicted_species, code, environment_1) %>% 
  summarise(N = sum(N)) %>%
  ungroup() %>% 
...
polycultures2 <- polycultures2 %>% 
  ungroup() %>% 
  droplevels() %>% 
  complete(temporal_change, dispersal, replicate, time, predicted_species, environment_1, code, fill = list(N = 0))


pop_data_0 <- pop_data_0 %>% dplyr::select(-predicted_missing)
pop_data_0_mono <- pop_data_0 %>% filter(code != "mix") %>% 
  mutate(code = "monoculture")
pop_data_0_polyculture <- pop_data_0 %>% filter(code == "mix") %>% 
  mutate(code = "polyculture")

#combine monocultures and polyculture data together in single dataframe
all_data <- bind_rows(monocultures, polycultures1,polycultures2)

#make descriptive names for treatments for plots
all_data <- all_data %>% 
  mutate(code = factor(code, levels = c("monoculture", "polyculture"), ordered = TRUE)) %>% 
...
initial_abund_expanded$code <- rep(rep(c("monoculture", "polyculture"), each = 4), 7*2)
initial_abund_expanded$environment_1 <- rep(rep(c("light", "dark"), each = 8), 7)

#timeseries####
time_series.df <- all_data %>% 
  group_by(dispersal, code, predicted_species, temporal_change, environment_1, time) %>% 
...
ggplot(time_series.df, aes(x = time, y = N, color = predicted_species, fill = predicted_species, group = interaction(predicted_species, dispersal), linetype = dispersal))+
  ...
  geom_line(size = 1)+
  facet_grid(code~environment_1*temporal_change)+
  scale_y_log10()+
  theme_classic()+
...
ggplot(pre_differences, aes(x = predicted_species, y = difference, color = code))+
...
ggsave("./figures/Figure S1.pdf", height = 4, width = 8)
...
all_data %>% 
  ...
  summarise(difference = N[code == "monoculture"] -  N[code == "polyculture"], 
            SE = sqrt((SD[code == "monoculture"]^2/size[code == "monoculture"]) + 
                        (SD[code == "polyculture"]^2/size[code == "polyculture"])))
...
all_data %>% 
  ...
  xlab("Species")+
  ylab(bquote('Individuals '~mL^-1))
ggsave("./figures/Figure S2.pdf", height = 4, width = 8)

#add index for metacommunity
all_data <- all_data %>% 
  group_by(dispersal, environment_1, temporal_change, code, replicate) %>% 
...
adonis(vegdist(decostand(N_wide, method = "total"), method = "bray")~ code * environment_1, data = data_end_wide)

all_data %>% 
  filter(time == 4, environment_1 == "dark") %>% 
  group_by(predicted_species, time, code, environment_1, temporal_change) %>% 
  summarise(SE = sd(N)/sqrt(n()), N = mean(N), size = n()) %>% 
  ungroup() %>% 
  group_by(predicted_species, environment_1, code) %>% 
...
adonis(vegdist(decostand(N_wide, method = "total"), method = "bray")~ temporal_change * code * environment_1  * dispersal, data = data_end_wide)
...
X <- pop_data_wide %>% select(temporal_change:code, env)
...
ggplot(filter(scores, time == 4), aes(x = NMDS1, y = NMDS2, fill = env, pch = temporal_change))+
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        strip.text = element_text(size=12),
        strip.background = element_rect(colour="white", fill="white"))
ggsave("./figures/protist_nmds.png", height = 6, width = 8)
...
centroids <- pop_data_wide %>% 
  filter(time == 4) %>% 
  group_by(temporal_change, dispersal, time, environment_1, code, env) %>% 
  summarise_each(mean)
...
for(i in 1:nrow(change_data)){
  ...
  match_centroid_1 <- centroids %>% 
    ungroup() %>% 
    filter(temporal_change == "no", dispersal == hold$dispersal, environment_1 == hold$environment_1, code == hold$code) %>% 
    select(col:tet)
  
  match_centroid_2 <- centroids %>% 
    ungroup() %>% 
    filter(temporal_change == "no", dispersal == hold$dispersal, env == hold$env, code == hold$code) %>% 
    select(col:tet)
  ...
  community_dist <- rbind(community_dist, data.frame(code = hold$code, dispersal = hold$dispersal, environment_1 = hold$environment_1, environment_2 = hold$env, centroid_dist = hold_dist[3], init_dist = hold_dist[1], final_dist = hold_dist[2], replicate = hold$replicate))
}

community_dist$metacommunity <- community_dist$replicate
community_dist$metacommunity[community_dist$code == "mixture"] <- community_dist$metacommunity[community_dist$code == "mixture"]+3

community_dist %>% 
  group_by(code, environment_2) %>% 
  summarise(mean = mean(final_dist), se = sd(final_dist)/sqrt(n()))

diff_means<-community_dist %>% 
  group_by(code, dispersal) %>% 
...
ggplot(community_dist, aes(x = interaction(dispersal, environment_2), y = final_dist, fill = dispersal)) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        strip.text = element_text(size=12),
        strip.background = element_rect(colour="white", fill="white"))
ggsave("./figures/tracking.pdf", width = 7,height = 4.5)


hist(community_dist$final_dist)
contrasts(community_dist$code)
contrasts(community_dist$code) <- contr.sum #adding control sum contrasts - to make the main effects interpretable
contrasts(community_dist$dispersal) <- contr.sum
contrasts(community_dist$environment_2) <- contr.sum
com.dist.lme <- lme(final_dist ~ code * environment_2 * dispersal , random = ~ 1|metacommunity, data = community_dist)
